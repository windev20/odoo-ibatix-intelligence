...
def _extract_pdf_text(pdf_bytes):
    ...
    try:
        ...
        _logger.warning("Échec extraction PDF : %s", e)
        return ''


def _clean_number(raw):
    """Supprime les espaces insécables et espaces dans une chaîne numérique."""
    return re.sub(r'[\s\u00a0\u202f]+', '', raw)


def _parse_avis_imposition(text):
    """
    Analyse le texte extrait d'un avis d'imposition français et retourne
    un dict avec les champs reconnus.
    """
    result = {}
    # Normalisation minimale : guillemets typographiques, tirets…
    t = text.replace('\u2019', "'").replace('\u2018', "'")

    # ── 1. Numéro fiscal (SPI) ── 13 chiffres, éventuellement espacés
    m = re.search(
    ...
    # ── 2. Référence de l'avis (ou référence fiscale)
    m = re.search(
    ...
    # ── 3. Année fiscale
    # Priorité : "revenus de l'année XXXX" ou "revenus XXXX"
    m = re.search(
        r'revenus?\s+(?:de\s+l[\'']ann[ée]e\s+|de\s+)?(\d{4})',
        t, re.IGNORECASE
    )
    if not m:
        # Fallback : "avis d'imposition … XXXX"
        m = re.search(
            r'(?:avis\s+d[\'']imposition|ann[ée]e\s+d[\'']imposition)[^0-9]{0,20}(\d{4})',
            t, re.IGNORECASE
        )
    if not m:
        # Fallback : "imposition 20XX" ou "20XX"  (on prend le premier 20xx trouvé)
        m = re.search(r'\b(20\d{2})\b', t)
    if m:
        result['annee_fiscale'] = int(m.group(1))

    # ── 4. Revenu fiscal de référence (montant entier en €)
    m = re.search(
    ...
    if m:
        ...
        try:
            # On arrondit au demi le plus proche (1, 1.5, 2, 2.5…) puis on stocke en entier
            val = float(raw)
            result['nb_parts_foyer'] = round(val)
        except ValueError:
            pass
    ...
    def action_parse_avis_imposition(self):
        ...
        for record in self:
            ...
            try:
                ...
                _logger.warning("Impossible de décoder le fichier pour la ligne %s : %s", record.id, e)
                continue
            ...
            if data:
                record.write(data)
                _logger.info("Avis d'imposition analysé pour la ligne %s : %s", record.id, data)
            else:
